...
const initGame = (button, clickedLetter, hintCon, keyboard, gameplayCon, hangmanImg, wordDisplay, guessDisplay) => {
	if (currentWord.includes(clickedLetter)) {
		[...currentWord].forEach((letter, index) => {
			if (letter === clickedLetter) {
				...
				elements.wordDisplay.querySelectorAll("li")[index].classList.add("guessed");
			}
		});
	} else {
		wrongGuessCount++;
	...
	if (currentWord.length === correctLetters.length) return winScreen(hintCon, keyboard, gameplayCon);
	if (wrongGuessCount === maxGuesses) return lossScreen(hintCon, keyboard, gameplayCon, wordDisplay, guessDisplay);
